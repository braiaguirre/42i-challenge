const findPairWithSum = require('../algorithms/twoNumberSum');
...
test('Case 3 / Pair exists in the middle of the array.', () => {
  const numbers = [3, 6, 8, 10];
  const targetSum = 14;
  expect(findPairWithSum(numbers, targetSum)).toEqual([6, 8]);
});
...
test('Case 5 / Pair exists at the end of the array.', () => {
  ...
  const targetSum = 16;
  expect(findPairWithSum(numbers, targetSum)).toEqual([7, 9]);
});
